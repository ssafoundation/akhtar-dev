          ...
          {faqs.map((item, index) => (
            ...
            >
              <span className="flex items-center justify-between gap-5">
                <span className="headline text-xl font-semibold text-white">{item.question}</span>
                <ChevronDown className={cn("h-5 w-5 shrink-0 text-accent transition", active === index && "rotate-180")} />
              </span>
              {active === index ? <span className="mt-4 block leading-7 text-muted">{item.answer}</span> : null}
            </button>
          ))}
        </div>
      </div>
